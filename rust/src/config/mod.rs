...
impl Config {
    ...
    pub async fn load(path: &str) -> Result<Self> {
        ...
        let mut builder = ConfigLib::builder()
            .add_source(File::from_str(include_str!("../../defaults.toml"), config::FileFormat::Toml))
        ...
        let conf_dir = config_path.parent()
...
fn default_name() -> String { "pwnagotchi-zero".to_string() }
fn default_lang() -> String { "en".to_string() }
fn default_iface() -> String { "wlan0mon".to_string() }
...
fn default_confd() -> String { "/etc/pwnagotchi/conf.d/".to_string() }
fn default_custom_plugins() -> String { "/usr/local/share/pwnagotchi/custom-plugins/".to_string() }
...
fn default_log_path() -> String { "/etc/pwnagotchi/log/pwnagotchi.log".to_string() }
fn default_log_debug_path() -> String { "/etc/pwnagotchi/log/pwnagotchi-debug.log".to_string() }
...
fn default_happy() -> Vec<String> { vec!["(•‿‿•)".to_string(), "(^‿‿^)".to_string(), "(^◡◡^)".to_string()] }
fn default_excited() -> Vec<String> { vec!["(ᵔ◡◡ᵔ)".to_string(), "(✜‿‿✜)".to_string()] }
fn default_grateful() -> Vec<String> { vec!["(^‿‿^)".to_string()] }
fn default_motivated() -> Vec<String> { vec!["(☼‿‿☼)".to_string(), "(★‿★)".to_string(), "(•̀ᴗ•́)".to_string()] }
fn default_demotivated() -> Vec<String> { vec!["(≖_≖)".to_string(), "(￣ヘ￣)".to_string(), "(¬_¬)".to_string()] }
fn default_smart() -> Vec<String> { vec!["(✜‿‿✜)".to_string()] }
fn default_lonely() -> Vec<String> { vec!["(ب_ب)".to_string(), "(｡•́︿•̀｡)".to_string(), "(︶︹︺)".to_string()] }
fn default_sad() -> Vec<String> { vec!["(╥☁╥ )".to_string(), "(╥﹏╥)".to_string(), "(ಥ﹏ಥ)".to_string()] }
fn default_angry() -> Vec<String> { vec!["(-_-')".to_string(), "(⇀_⇀)".to_string(), "(`___´)".to_string()] }
fn default_friend() -> Vec<String> { vec!["(♥‿‿♥)".to_string(), "(♡‿‿♡)".to_string(), "(♥‿♥ )".to_string(), "(♥ω♥ )".to_string()] }
fn default_broken() -> Vec<String> { vec!["(☓‿‿☓)".to_string()] }
fn default_debug() -> Vec<String> { vec!["(#_#)".to_string()] }
fn default_upload() -> Vec<String> { vec!["(1_0)".to_string(), "(1_1)".to_string(), "(0_1)".to_string()] }
fn default_frame_padding() -> bool { true }
fn default_frame_padding_min_bytes() -> usize { 650 }
...
fn default_web_address() -> String { "::".to_string() }
...
fn default_handshakes_path() -> String { "/etc/pwnagotchi/handshakes".to_string() }
...
fn default_pisugar_addr() -> u16 { 0x24 }
fn default_pisugar_button_long() -> u32 { 3 }
fn default_pisugar_watchdog() -> u32 { 30 }
fn default_wifi_recovery_gpio() -> u32 { 4 }
fn default_epoch_duration() -> u32 { 30 }
fn default_attack_rate() -> u32 { 1 }
fn default_dc_pin() -> u32 { 25 }
fn default_rst_pin() -> u32 { 17 }
fn default_busy_pin() -> u32 { 24 }
